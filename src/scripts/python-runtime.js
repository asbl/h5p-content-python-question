...
export default class PythonRuntime extends H5P.Runtime {
  ...
  constructor(question, options) {
    ...
    this.console = null;
    this.editor = null;
    this.inputTakesPrompt = options.inputTakesPrompt !== undefined ? options.inputTakesPrompt : true;
    this.saveOutput = (options.saveOutput !== undefined) ? options.saveOutput : false;
    this.stopped = false; // set in this.run / this.stop
    this.suspension = (options.shouldStop !== undefined) ? {
      '*': () => {
        if (this.stopped === true) throw 'Program suspended';
      }
    } : {};
    this.killableWhile = (options.shouldStop !== undefined && options.killableWhile !== undefined) ? options.killableWhile : true;
    this.killableFor = (options.shouldStop !== undefined && options.killableFor !== undefined) ? options.killableFor : true;
  ...
  _containsCanvasCode() {
    if (this.code.includes('import turtle') || 
  ...
  onError(error) {
    ...
    try {
      ...
      this.outputHandler('Error: ' + errorMessage + ' on line ' + lineNumber, '; column: ' + colNumber);
      // Handle on Error
