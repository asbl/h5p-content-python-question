...
export default class PythonTestRuntime extends PythonRuntime {
  ...
  setCanvas() {
    ...
    const testCaseTableCell = this.codeTester
      .getTestCasesArea()
      .querySelector(`.user-output.output-${this.codeTester.testCaseCounter}`);
  ...
  async runSolution() {
    if (this.codeTester.runSolution) {
      const solutionRuntime = new H5P.PythonSolutionRuntime(
        this.question,
        this.codeTester,
        this.options,
      );
  ...
  async runNextTest() {
    ...
    const testruntime = this.question.factory.createTestRuntime(
      this.codeTester,
      this.code,
    );
